...
export default function FollowersPage() {
  ...
  const fetchList = async (query?: string) => {
    setLoading(true);
    const url = `/api/users/${username}/followers` + (query ? `?q=${encodeURIComponent(query)}` : "");
    const res = await fetch(url);
    const data = await res.json();
    if (res.ok) setList(data);
    else {
      // unauthorized or hidden
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-lg mx-auto px-4 py-6 page-transition">
        <h1 className="text-lg font-bold mb-4">{username} - Takipçiler</h1>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            ...
            {list.length === 0 && <div className="text-gray-500">Takipçi yok veya gizli.</div>}
          </div>
        )}
      </main>
    </div>
