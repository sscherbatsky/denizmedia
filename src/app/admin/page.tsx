...
export default function AdminPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setStats(statsData);
      try {
        const wl = await fetch("/api/admin?action=whitelist");
        const wlJson = await wl.json();
        if (!wlJson.error) {
          setAllowedIps(wlJson.ips || []);
          setOwnerEmail(wlJson.ownerEmail || null);
        }
      } catch (e) {
        // ignore
      }
  ...
  const addIp = async (ip: string) => {
    ...
    try {
      const res = await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "whitelist_add_ip", ip }),
      });
      const j = await res.json();
      setMessage(j.message || j.error || "");
      if (j.ips) setAllowedIps(j.ips);
    } catch (e) {
  ...
  const removeIp = async (ip: string) => {
    ...
    try {
      const res = await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "whitelist_remove_ip", ip }),
      });
      const j = await res.json();
      setMessage(j.message || j.error || "");
      if (j.ips) setAllowedIps(j.ips);
  ...
  const handleModalSubmit = () => {
    ...
    } else if (modal.type === "set_password") {
      doAction("set_password", modal.userId, { newPassword: modalInput });
    }
    setModal(null);
    setModalInput("");
  };
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-sm mx-4 shadow-xl">
            ...
            <div className="flex gap-2">
              <button onClick={() => { setModal(null); setModalInput(""); }} className="flex-1 bg-gray-100 text-gray-600 py-2.5 rounded-xl text-sm hover:bg-gray-200 transition">İptal</button>
              <button onClick={handleModalSubmit} className="flex-1 bg-red-600 text-white py-2.5 rounded-xl text-sm hover:bg-red-700 transition font-medium">Onayla</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold"><span className="text-gray-900">Admin</span> <span className="text-red-500">Panel</span></h1>
          <button onClick={() => router.push("/feed")} className="text-gray-400 hover:text-gray-900 text-sm transition">← Ana Sayfa</button>
        </div>

        {message && <div className="bg-blue-50 border border-blue-200 text-blue-600 text-sm rounded-xl p-3 mb-6">{message}</div>}
          ...
          {users.map((user) => (
            <div key={user.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-sm font-bold text-blue-600 overflow-hidden">
                    {user.profileImage ? <img src={user.profileImage} alt="" className="w-full h-full object-cover" /> : user.username[0]?.toUpperCase()}
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="font-semibold text-gray-900 text-sm">{user.displayName || user.username}</span>
                      {user.isVerified && (
                        <svg className="w-4 h-4 text-blue-500" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>
                      )}
                      ...
                      {user.timeoutUntil && new Date(user.timeoutUntil) > new Date() && <span className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">TIMEOUT</span>}
                    </div>
              ...
              <div className="flex flex-wrap gap-2">
                {user.isBanned ? (
                  <ActionBtn onClick={() => doAction("unban", user.id)} loading={actionLoading === user.id + "unban"} color="green">Ban Kaldır</ActionBtn>
                ) : (
                  <ActionBtn onClick={() => { setModal({ type: "ban", userId: user.id }); setModalInput(""); }} loading={actionLoading === user.id + "ban"} color="red">Banla</ActionBtn>
                )}

                {user.isVerified ? (
                  <ActionBtn onClick={() => doAction("unverify", user.id)} loading={actionLoading === user.id + "unverify"} color="gray">Tik Kaldır</ActionBtn>
                ) : (
                  <ActionBtn onClick={() => doAction("verify", user.id)} loading={actionLoading === user.id + "verify"} color="blue">Mavi Tik Ver</ActionBtn>
                )}

                <ActionBtn onClick={() => { setModal({ type: "timeout", userId: user.id }); setModalInput(""); }} loading={actionLoading === user.id + "timeout"} color="orange">Timeout</ActionBtn>

                {user.timeoutUntil && new Date(user.timeoutUntil) > new Date() && (
                  <ActionBtn onClick={() => doAction("remove_timeout", user.id)} loading={actionLoading === user.id + "remove_timeout"} color="gray">Timeout Kaldır</ActionBtn>
                )}

                {user.isRestricted ? (
                  <ActionBtn onClick={() => doAction("unrestrict", user.id)} loading={actionLoading === user.id + "unrestrict"} color="green">Engel Kaldır</ActionBtn>
                ) : (
                  <ActionBtn onClick={() => doAction('restrict', user.id)} loading={actionLoading === user.id + "restrict"} color="yellow">Erişim Engelle</ActionBtn>
                )}
                ...
                <ActionBtn onClick={() => { setModal({ type: 'delete_user_posts', userId: user.id }); setModalInput(''); }} loading={actionLoading === user.id + 'delete_user_posts'} color="red">Gönderilerini Sil</ActionBtn>
              </div>
            </div>
          ))}

          {users.length === 0 && (
            <div className="text-center py-12 text-gray-500"><p>Henüz kayıtlı kullanıcı yok.</p></div>
          )}
        </div>
      </div>

      {session?.user?.email && ownerEmail === session.user.email && (
        <div className="mt-6 max-w-4xl mx-auto p-4 bg-white border border-gray-200 rounded-2xl shadow-sm">
          <h3 className="font-semibold mb-2">Admin IP Whitelist</h3>
          <p className="text-sm text-gray-500 mb-3">Bu listeye eklenen IP'ler admin yetkisi alır. Sadece sahibi bu listeyi değiştirebilir.</p>
          <div className="flex gap-2 mb-3">
            <input value={newIp} onChange={(e)=>setNewIp(e.target.value)} placeholder="IP adresi (örn. 1.2.3.4)" className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm" />
            <button onClick={()=>{ if(newIp) { addIp(newIp); setNewIp(""); } }} className="bg-blue-600 text-white px-4 py-2 rounded-xl">Ekle</button>
          </div>
          <div className="flex flex-wrap gap-2">
            {allowedIps.map((ip)=> (
              <div key={ip} className="px-3 py-1 rounded-full bg-gray-100 border border-gray-200 text-sm flex items-center gap-2">
                <span>{ip}</span>
                <button onClick={()=>removeIp(ip)} className="text-red-500 text-xs">Kaldır</button>
              </div>
            ))}
            {allowedIps.length === 0 && <div className="text-sm text-gray-500">Whitelist boş.</div>}
          </div>
        </div>
      )}
    </div>
...
function ActionBtn({ onClick, loading, color, children }: { onClick: () => void; loading: boolean; color: string; children: React.ReactNode }) {
  ...
  return (
    <button onClick={onClick} disabled={loading} className={`${colors[color] || colors.gray} border text-xs px-3 py-1.5 rounded-lg transition disabled:opacity-50`}>
      {loading ? "..." : children}
    </button>
  );
}
...
interface AdminUser {
  ...
  email: string;
                <div className="flex flex-wrap gap-2">
                  ...
                  <ActionBtn
                    onClick={() => { setModal({ type: 'set_password', userId: user.id }); setModalInput(''); }}
                    loading={actionLoading === user.id + 'set_password'}
                    color="gray"
                  >
                    Şifre Ayarla
                  </ActionBtn>
  ...
  const handleModalSubmit = () => {
    ...
    if (modal.type === "ban") {
      ...
      // no length restriction by design — admin can set any password
      doAction("set_password", modal.userId, { newPassword: modalInput });
    }
    setModal(null);
    setModalInput("");
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Whitelist management - visible to owner only */}
      {session?.user?.email && ownerEmail === session.user.email && (
        <div className="mt-6 max-w-4xl mx-auto p-4 bg-white border border-gray-200 rounded-2xl shadow-sm">
          <h3 className="font-semibold mb-2">Admin IP Whitelist</h3>
          <p className="text-sm text-gray-500 mb-3">Bu listeye eklenen IP'ler admin yetkisi alır. Sadece sahibi bu listeyi değiştirebilir.</p>
